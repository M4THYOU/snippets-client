...
export function insertIntoString(s1: string, i: number, s2: string) {
    if (i > 0) {
...
export function rawToRawString(raw: IRawPart[]) { // TODO
    let s = '';
    raw.forEach(part => {
        if (part.isMath) {
...
export function parseRawString(s: string) {
    const regMatch = s.split(/`(.*?)`/);
    if (s.length === 0) {
        return [];
    }
    let isMath = false;
    if ((s.charAt(0) === '`') && (s.charAt(1) !== '`') && (regMatch[0] === '')) {
        isMath = true;
    }
    let arr: IRawPart[] = [];
...
function isMathAtIndex(index: number, s: string) {
